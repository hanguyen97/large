...
//' @return a list containing  variables to ignore because they are highly correlated with other, and SLR coefficients
// [[Rcpp::export]]
List glasso_autotune(arma::mat X, double alpha = 0.1, double thr = 1e-4, 
                      int maxit = 1e4) {
   ...
   for (int iter = 1; iter <= maxit; iter++) {
     ...
     for (int j = 0; j < p; j++) {
       ...
       arma::mat W_11 = W.submat(idx, idx);
       arma::vec s_12 = S.submat(idx, uvec{(unsigned int)j});
       double s_22 = S(j, j);
       ...
       W.submat(idx, uvec{(unsigned int)j}) = W_11 * b_hat;
       W.submat(uvec{(unsigned int)j}, idx) = trans(W_11 * b_hat);
       
       if (final_cycle) {
         Theta(j, j) = 1.0 / (W(j, j) - dot(W.submat(idx, uvec{(unsigned int)j}), b_hat));
         Theta.submat(idx, uvec{(unsigned int)j}) = -Theta(j, j) * b_hat;
         Theta.submat(uvec{(unsigned int)j}, idx) = trans(-Theta(j, j) * b_hat);
       }
